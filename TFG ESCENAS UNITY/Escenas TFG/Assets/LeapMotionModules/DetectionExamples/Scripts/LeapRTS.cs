...
//      }
    ...
    void OnGUI() {
      if (_showGUI) {
        GUILayout.Label("Una mano");
        doRotationMethodGUI(ref _oneHandedRotationMethod);
        GUILayout.Label("Dos manos");
        doRotationMethodGUI(ref _twoHandedRotationMethod);
        _allowScale = GUILayout.Toggle(_allowScale, "Escalar 2 manos");
      }
    }
